...
control heavy_hitter ( inout headers_t H)  {
  ...
  bit<32> c3;
  bit<32> c4;
  action compute_cms_idx_1() { c1idx = cms_hash_1.get({H.cache.k}); }
  action compute_cms_idx_2() { c2idx = cms_hash_2.get({H.cache.k}); }
  action compute_cms_idx_3() { c3idx = cms_hash_3.get({H.cache.k}); }
  action compute_cms_idx_4() { c4idx = cms_hash_4.get({H.cache.k}); }
  ...
  bit<8> b2;
  bit<8> b3;
  action compute_bf_idx_1() { bidx1 = bh1.get({H.cache.k}); }
  action compute_bf_idx_2() { bidx2 = bh2.get({H.cache.k}); }
  action compute_bf_idx_3() { bidx3 = bh3.get({H.cache.k}); }

  bit<32> cond1;
  bit<32> cond2;
  bit<32> cond3;
  bit<32> cond4;
  action compute_cond_1() { cond1 = HH_THRESH - c1; }
  action compute_cond_2() { cond2 = HH_THRESH - c2; }
  action compute_cond_3() { cond3 = HH_THRESH - c3; }
  action compute_cond_4() { cond4 = HH_THRESH - c4; }
  action hot() { H.cache.hot = 1; }
  action cold() { H.cache.hot = 0; }
  table check {
    key = { b1: ternary; b2: exact; b3: exact; }
    actions = {hot; cold;}
...
control netcache ( inout headers_t H,
  ...
  action read_idx(idx_t i) { idx = i; }
  table index {
    key = { H.cache.k: exact; }
    actions = { read_idx; NoAction; }
    default_action = NoAction;
    size = CACHE_SIZE;
  }

  action read_mask(bitmap_t mask) { H.cache.mask = mask; }
  table mask {
    key = { H.cache.k: exact;}
    actions = { read_mask; NoAction; }
    default_action = NoAction;
    size = CACHE_SIZE;
  }
  ...
  action read_valid_lo() { valid = reg_valid_read_lo.execute(idx); }
  action read_valid_hi() { valid = reg_valid_read_hi.execute(idx); }
  action set_valid() { valid = reg_valid_set_valid.execute(idx); }
  action set_invalid() { valid = reg_valid_set_invalid.execute(idx); }

  table chk_valid {
    key = { H.cache.mask: ternary; }
    actions = { read_valid_lo; read_valid_hi; }
    const entries = {
      1 &&& 1 : read_valid_lo();
            _ : read_valid_hi();
    }
  ...
  action read_cache_1_action() { H.cache.v1 = read_cache_1.execute(idx); }
  action read_cache_2_action() { H.cache.v2 = read_cache_2.execute(idx); }
  action read_cache_3_action() { H.cache.v3 = read_cache_3.execute(idx); }
  action read_cache_4_action() { H.cache.v4 = read_cache_4.execute(idx); }
  action write_cache_1_action() { write_cache_1.execute(idx); }
  action write_cache_2_action() { write_cache_2.execute(idx); }
  action write_cache_3_action() { write_cache_3.execute(idx); }
  action write_cache_4_action() { write_cache_4.execute(idx); }
