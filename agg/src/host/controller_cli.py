...
def configure_switch_endpoint(cfg):
    ...
    err = None
    sw_mac = EUI(cfg['switch']['mac'])
    sw_ip4 = IPAddress(cfg['switch']['ip'])
    try:
...
def configure_switch_arp_resolver(cfg):
...
def configure_switch_forwarding(cfg):
...
def configure_multicast(cfg):
    ...
    for mgid in cfg['switch']['multicast']:
        ...
        try :
            ...
            rid = rid + 1
            nid = nid + 1
            print("  %2d ->" % int(mgid), ports, ", %s" % cfg['switch']['multicast'][mgid]['desc'])
        except Exception as e:
            print(mgid, ports, e)
